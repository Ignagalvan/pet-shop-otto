...
export async function POST(request: Request) {
  ...
  try {
    const parsed = await parseOttoWorkbook(await file.arrayBuffer())

    if (parsed.summary.errors > 0) {
      return NextResponse.json(
        { error: 'El archivo contiene errores que deben corregirse antes de importarlo.' },
        { status: 422 },
      )
    }

    const { data, error } = await supabase.rpc('apply_otto_import', {
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : 'No pudimos aplicar la importación.',
      },
      { status: 422 },
    )
  }
